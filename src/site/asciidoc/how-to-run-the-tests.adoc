...
`java -jar ets-ogcapi-features-1.0-${version}-aio.jar [-o|--outputDir $TMPDIR] [test-run-props.xml]`
...
You need to retrieve the https://hub.docker.com/r/ogccite/ets-ogcapi-features-1.0[image from Docker Hub] by running the following

    docker run -p 8081:8080 ogccite/ets-ogcapi-features-1.0
...
Choose 'OGC' organization then Specification 'OGC API Features - 1.0 [ 0.4 ]'
