...
public class FeaturesCrsParameterTransform extends AbstractFeaturesCrs {
	...
	@Test(description = "Implements A.2.1 Query, Parameter crs, Abstract Test 7 (Requirement /req/crs/crs-action), "
			+ "Geometries in the path /collections/{collectionId}/items", dataProvider = "collectionIdAndJson",
	...
	public void verifyFeaturesCrsParameterTransformWithoutCrsParameter(String collectionId, JsonPath collection)
		...
		if (crsHeader == null) {
			throw new AssertionError(String
				.format("Features response at '%s' does not provide the expected header 'Content-Crs'", featuresUrl));
		}
		assertDefaultCrs(crsHeader, String.format(
				"Features response at '%s' does not provide default 'Content-Crs' header, was: '%s', expected: '%s' or '%s'",
	 ...
	 */
	@Test(description = "Implements A.2.1 Query, Parameter crs, Abstract Test 7 (Requirement /req/crs/crs-action), "
			+ "Transformed geometries in the path /collections/{collectionId}/items",
			dataProvider = "collectionIdAndJsonAndCrsAndDefaultCrs", dependsOnGroups = "crs-conformance",
			dependsOnMethods = "verifyFeaturesCrsParameterTransformWithoutCrsParameter", priority = 1)
	public void verifyFeaturesCrsParameterTransformWithCrsParameter(String collectionId, JsonPath collection,
			CoordinateSystem crs, CoordinateSystem defaultCRS) throws ParseException {
