...
public class JsonUtilsTest {
	...
	public static void parseJson() {
		InputStream collectionJson = JsonUtilsTest.class
			.getResourceAsStream("../conformance/core/collections/collections.json");
		jsonCollection = new JsonPath(collectionJson);
		InputStream collectionItemsJson = JsonUtilsTest.class
	...
	@Test
	public void testFindFeaturesUrlForGeoJson() throws Exception {
		InputStream collectionJson = JsonUtilsTest.class
			.getResourceAsStream("../conformance/core/collections/collection-flurstueck.json");
	...
	@Test
	public void testFindFeatureUrlForGeoJson() throws Exception {
		...
		assertThat(featuresUrlForGeoJson, is("https://test.service.de/ogcapi/datasets/kitaeinrichtung/items/testId"));
	}

	@Test
	public void testFindFeatureUrlForGeoJson_withParam() throws Exception {
		Map<String, Object> link = createItemsLinks(
				"https://test.service.de/ogcapi/datasets/kitaeinrichtung/items?f=json");
		JsonPath collection = Mockito.mock(JsonPath.class);
		when(collection.get("links")).thenReturn(Collections.singletonList(link));
		URI rootUri = new URI("http://localhost:8090/rest/services");
		String featuresUrlForGeoJson = findFeatureUrlForGeoJson(rootUri, collection, "testId");
	...
	@Test
	public void testFindFeatureUrlForGeoJson_withExtension() throws Exception {
		Map<String, Object> link = createItemsLinks(
				"https://test.service.de/ogcapi/datasets/kitaeinrichtung/items.json");
		JsonPath collection = Mockito.mock(JsonPath.class);
		when(collection.get("links")).thenReturn(Collections.singletonList(link));
		URI rootUri = new URI("http://localhost:8090/rest/services");
		String featuresUrlForGeoJson = findFeatureUrlForGeoJson(rootUri, collection, "testId");
	...
	@Test
	public void testCollectNumberOfAllReturnedFeatures() throws Exception {
		prepareJadler();
	...
	@Test
	public void testParseFeatureGeometry() throws Exception {
	...
	private void prepareJadler() {
		...
		onRequest().havingParameter("startindex", allOf(notNullValue(), not(hasItems("10")), not(hasItems("20"))))
			.respond()
			.withBody(item31_40);
	}
